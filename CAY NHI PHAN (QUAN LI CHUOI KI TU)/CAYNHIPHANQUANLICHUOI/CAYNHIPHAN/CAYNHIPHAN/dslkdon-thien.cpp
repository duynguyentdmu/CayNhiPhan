...
//Khai báo biến con trỏ chỉ đến một nút
typedef node *NODEPTR;
//Quản lý dsld đơn qua con trỏ đầu plist
NODEPTR *plist;
//	Một số tác vụ
//Tác vụ getnode:
NODEPTR getnode()
...
//Tác vụ initialize: Khởi tạo danh sách rỗng
...
//Tác vụ nodepointer:
NODEPTR nodepointer(NODEPTR *plist, int i) // Dùng để xác định vị trí con trỏ
{
	NODEPTR p;
	int vitri = 0;
	p = *plist;
...
//Tác vụ push: Thêm một node vào đầu danh sách
...
//Tác vụ insafter: Thêm 1 node vào sau node p
...
//Tác vụ pop: xóa node đầu khỏi danh sách
...
//Tác vụ delafter: xóa 1 node sau node p
...
//Tác vụ place: thêm một nút có nội dung x vào dslk đã có thứ tự
...
//Tác vụ traverse: Duyệt ds và in thông tin
...
//Tác vụ clearlist: xóa toàn bộ danh sách
